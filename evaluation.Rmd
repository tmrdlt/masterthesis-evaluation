...
# Perform one-sample t-test to check if mean differs signficantly from standard value of 2
# 1. sample size < 30 -> Check if data is normally distributed via Shapiro-Wilk's test
# 2. 
# 3. Outliers? -> Inspect Boxplot
# http://www.sthda.com/english/wiki/one-sample-t-test-in-r
# 

library("ggpubr")
ggqqplot(level$`Favourite of proposed`, ylab = "Personal nesting level",
...
median(level$`Favourite of proposed`)

# http://www.sthda.com/english/wiki/wiki.php?title=one-sample-wilcoxon-signed-rank-test-in-r
shapiro.test(level$`Favourite of proposed`)
# One-sample Wilcoxon signed rank test because data is not normally distributed
...
```
## Task 2 - Manual scheduling evaluation - Raw data
```{r}
library(readxl)
library(dplyr)
library(tidyr)
...
plot1 <- data1 %>%
  ...
  geom_hline(yintercept=c(as.POSIXct("2021-11-01 19:00:00")), linetype="dotted")+
...
plot2 <- data2 %>%
  ...
  geom_line(aes(y=value, x=variable, color=variable, group=`Participant ID`), position = position_jitter(w = 0.2, h = 0, seed = 1)) +
  geom_point(aes(y=value, x=variable, color=variable, group=`Participant ID`), position = position_jitter(w = 0.2, h = 0, seed = 1)) +  geom_hline(yintercept=c(as.POSIXct("2021-11-05 19:00:00")), linetype="dotted")+
...
ggsave("plots/box_diff_order.eps",
       ...
       limitsize = TRUE)
```
## Task 2 - Signicifance of deviation from machine planning
```{r}
library(readxl)
library(stats)
